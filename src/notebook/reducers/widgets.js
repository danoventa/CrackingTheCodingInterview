...
export default {
  ...
  [constants.DELETE_WIDGET]: function deleteWidget(state, action) {
    return state.deleteIn(['widgetModels', action.id]);
  },
  ...
  [constants.DISPLAY_WIDGET]: function displayWidget(state, action) {
    ...
    return state.updateIn(['widgetViews', cellId], new Immutable.List(),
      views => views.push(id));
  },

  [constants.CLEAR_WIDGETS]: function clearWidgets(state, action) {
    const { id } = action;
    return state.setIn(['widgetViews', id], new Immutable.List());
  },
};
